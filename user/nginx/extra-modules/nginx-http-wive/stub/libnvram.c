...
struct auth_session_t *session = NULL;
...
//    nvram_stub_list = ngx_array_create(pool, 16, sizeof(keyval_t));
    ...
    while ((read = getline(&line, &len, fp)) != -1) {
        ...
        *ptr = '\0';
        ...
        fb[index].cache[i].name = strdup(line);
        fb[index].cache[i].value = strdup(ptr);
        i++;
...
static char *__nvram_bufget(int index, char *name)
{
	...
	if (session != NULL) {
		for (i=0;i<session->nvram_keyval->nelts;i++)
		{
			keyval_t elem = ((keyval_t*)session->nvram_keyval->elts)[i];
			if (elem.key != NULL && strcmp(elem.key, name) == 0) return elem.val;
		}
	}
...
//			keyval_t elem = ((keyval_t*)session->nvram_keyval->elts)[i];
			keyval_t* elem = ((keyval_t*)session->nvram_keyval->elts) + i;
			if (elem->key != NULL && strcmp(elem->key, name) == 0) {
				elem->val = strdup(value);
				return 0;
			}
		}

		keyval_t* keyval = ngx_array_push(session->nvram_keyval);
		keyval->key = strdup(name);
		keyval->val = strdup(value);
	}

/*
	int idx = cache_idx(fb[index].cache, name);
...
size_t nvram_get_tuple(int index, char *key, unsigned elem_index, char *value, size_t value_size)
{
    ...
    len = (e - s);
    if (len && value) {
        snprintf(value, value_size, "%.*s", (int)len, s);
    }
    return len;
}
...
void nvram_set_tuple(int index, char *key, unsigned elem_index, const char *value)
{
    ...
    if (i) {
        ...
        snprintf(nv, len, "%.*s%s%s", (int)(s - v), v, value, e);
    }
...
/*
	int i;
	FILE *f = fopen(NV_DEV, "wt");
	if (!f) return -1;
	fprintf(f, "#The following line must not be removed!\n");
	fprintf(f, "Default\n");


	for (i = 0; i < MAX_CACHE_ENTRY; i++)
	{
		if (fb[index].cache[i].name != NULL && fb[index].cache[i].value != NULL) {
...
int nvram_load_default(void)
{
	...
	if (atoi(MngmtStoreSettings) == 1) {
		...
		nvram_bufset(RT2860_NVRAM, "RemoteSSH", RemoteSSH);
	    }
