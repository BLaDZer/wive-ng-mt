		...
		<script>
			...
			function initValues() {
				...
				if (BUILD_DATE == '1') {
					...
					form.ntpcurrenttime.value		= cTime.toLocaleTimeString("ru-RU") + ' ' + cTime.toLocaleDateString("en-US"); 
				}
				else {
			...
			function syncWithHost() {
				var cTime	= new Date();
				var time	= cTime.toISOString().slice(0,10).replace(/-/g,"") + cTime.toLocaleTimeString("ru-RU").slice(0,5).replace(/:/g,"") + '.' + cTime.getSeconds(); 
