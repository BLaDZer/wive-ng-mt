 ...
 */
static struct t_ipt_account_table *ipt_account_table_init(struct t_ipt_account_info *info)
{
  ...
  if (!table->pde) {
    printk(KERN_ERR "ipt_account [ipt_account_table_init]: table->pde = create_proc_entry(table->name, S_IWUSR | S_IRUSR, ipt_account_procdir); failed.\n");
    goto cleanup_stats;
  }
