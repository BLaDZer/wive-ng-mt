...
static inline unsigned int skb_is_ready(struct sk_buff *skb)
{
	 ...
	 */
	if (!skb_cloned(skb) || skb_clone_writable(skb, 0))
		return 1;
...
static struct nf_conn __fastpathnet *
		...
		struct sk_buff *skb)
{
	...
	*/
	if (FASTNAT_SKIP_PROTO(protonum) || FASTNAT_SKIP_TYPE(skb) || pf != PF_INET || !skb_is_ready(skb)) {
		skip_offload = SKIP_SWO;		/* skip software offload */
		goto skip_alg_of;
	} else {
	    /* check pure route or nat ? */
	    ...
	    */
    	    if (nf_conntrack_fastroute && pure_route && FASTNAT_ESTABLISHED(ctinfo)) {
...
#endif
#if defined(CONFIG_RA_HW_NAT) || defined(CONFIG_RA_HW_NAT_MODULE) || defined(CONFIG_BCM_NAT_FASTPATH)
	...
	if (ra_sw_nat_hook_tx != NULL) {
		...
		EXAMPLE_CODE:
		if (!skip_offload && need ... rules ...) {
	    	    skip_offload = SKIP_ALL; (or SKIP_SWO for only software pass)
		    goto skip_alg_of;
		}
	    */
	}
