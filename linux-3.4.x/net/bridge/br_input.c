...
int __fastpathbridge br_handle_frame_finish(struct sk_buff *skb)
{
...
/* note: already called with rcu_read_lock */
static inline int br_handle_local_finish(struct sk_buff *skb)
{
	struct net_bridge_port *p = br_port_get_rcu(skb->dev);
 ...
 * note: already called with rcu_read_lock
 */
rx_handler_result_t __fastpathbridge br_handle_frame(struct sk_buff **pskb)
{
