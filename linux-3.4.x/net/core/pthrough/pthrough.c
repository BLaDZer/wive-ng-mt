...
static struct proc_dir_entry *pt_de_ipv6 = NULL;
...
extern int proc_ipv6_read(char *page, char **start, off_t offset, int count, int *eof, void *data);
extern int proc_ipv6_write(struct file *file, const char __user *buffer, unsigned long count, void *data);
...
void pthrough_remove_proc_entry(struct net *net)
{
	...
	if (pt_de_ipv6) {
		remove_proc_entry("ipv6", pt_de_root);
		pt_de_ipv6 = NULL;
	}
	if (pt_de_root) {
...
int pthrough_create_proc_entry(struct net *net)
{
	...
	/* create entry for ipv6 */
	pt_de_ipv6 = create_proc_entry("ipv6", 0644, pt_de_root);
	if (!pt_de_ipv6)
		goto pt_out;

	pt_de_ipv6->read_proc = proc_ipv6_read;
	pt_de_ipv6->write_proc = proc_ipv6_write;
