 ...
 *	%GFP_ATOMIC.
 */
struct sk_buff __fastpathnet *__alloc_skb(unsigned int size, gfp_t gfp_mask,
			    int fclone, int node)
{
...
static void __fastpathnet skb_release_data(struct sk_buff *skb)
{
	if (!skb->cloned ||
 ...
 */
static void __fastpathnet kfree_skbmem(struct sk_buff *skb)
{
