...
###################################################################################################################
eval `nvram_buf_get 2860 OperationMode FastRoaming \
			 ApProbeRspTimes AuthRspFail AuthRspRssi AssocReqRssiThres AssocRspIgnor \
			 KickStaRssiLow ProbeRspRssi`

is_up=`ip link show $1 up`
if [ "$is_up" = "" ] || [ "$FastRoaming" != "1" ]; then
    exit 0
fi

echo ">>>>> RECONFIGURE ROAMING WIFI IF = $1 <<<<<<<<<<"
###################################################################################################################
if [ "$OperationMode" = "0" ] || [ "$OperationMode" = "1" ] || [ "$OperationMode" = "3" ]; then
